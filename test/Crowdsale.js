...
describe('Crowdsale', () => {
  ...
  beforeEach(async () => {
    ...
    token = await Token.deploy('GhostRogue', 'GSTRG', '1000000000')

    accounts = await ethers.getSigners()
    deployer = accounts[0]
    user1 = accounts[1]

    crowdsale = await Crowdsale.deploy(token.address, ether(1), '1000000', SALE_START)
  ...
  describe('Finalzing Sale', () => {
    ...
    describe('Success', () => {
      ...
      it('transfers remaining tokens to owner', async () => {
        ...
        expect(await token.balanceOf(deployer.address)).to.equal(tokens(999999990))
      })
